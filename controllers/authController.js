...
exports.Usersignup = async (req, res) => {
  try {
    ...
    // const safeAddress = xss(req.body.address);
  ...
  try {
    ...
    if (user.status === "suspended") {
      if (user.suspendedUntil && new Date() > user.suspendedUntil) {
        ...
        return res.status(403).json({
          status: false,
          msg: "Your account is suspended until " + user.suspendedUntil,
        });
      }
    }
...
exports.verifyEmail = async (req, res) => {
  try {
    ...
    res
      .status(400)
      .send(
        "<h2>Invalid or expired token </h2><p>Please request a new verification email.</p>"
      );
  }
};
